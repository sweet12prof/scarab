...
std::map<uns64, ctype_pin_inst> generate_bp_limited_workload() {
  std::map<uns64, ctype_pin_inst>  workload;
  ...
  while (length_counter != workloadLength) {
    if (instr_count != (uns64)ISSUE_WIDTH - 1) {
      ...
      pc += nop_size;
        ...
        workload.insert({
                          pc, 
                          generate_conditional_branch(pc, uid, tgt_addr, (bool)branch_directions.get_next_element_in_distribution(), branch_size)
                        });
        instr_count = 0;
        pc = tgt_addr;
    }
  ...
  workload.insert({ 
                    pc, 
                    generate_unconditional_branch(pc, uid++, 0, branch_size)
                  });
  return workload;
}