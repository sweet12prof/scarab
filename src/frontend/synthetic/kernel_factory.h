...
class Kernel_Factory {
  ...
  // Top level helper
  void generate_kernel(Limit_Load_To level = DCACHE_LEVEL, uns num_of_dependence_chains = 1);

  /* Downstream configurable helpers */
  void generate_ubr_kernel(Sequence_Pick_Strategy branch_target_pick_strategy, uns64 target_pool_size,
                           uns64 workload_length, uns64 start_pc, uns64 start_uid, uns64 starting_target,
                           uns64 target_stride);

  void generate_ilp_kernel(uns dependence_chain_length, uns workload_length, uns64 start_pc, uns64 start_uid);

  void generate_load_kernel(Load_Kernel_Type type, uns workload_length, Sequence_Pick_Strategy mem_address_pick_srategy,
                            uns64 start_mem_address, uns64 mem_addresses_stride, Limit_Load_To level, uns64 start_pc,
                            uns64 start_uid);

  void generate_ibr_kernel(Sequence_Pick_Strategy branch_target_pick_strategy, uns64 target_pool_size, uns64 start_pc,
                           uns64 start_uid, uns64 target_stride, uns64 starting_target);

  void generate_icache_kernel(uns64 start_pc, uns64 start_uid);

  void generate_cbr_kernel(Sequence_Pick_Strategy branch_direction_pick_strategy,
                           Sequence_Pick_Strategy branch_target_pick_strategy, uns64 direction_pool_size,
                           double branch_t_nt_ratio, uns64 workload_length, uns64 start_pc, uns64 start_uid);

  // generates a sequence of nops and returns the location the next instruction starts from
  uns64 generate_nop_sequence(Sampler& uid_sequence, uns num_of_nops, uns64 starting_pc);
