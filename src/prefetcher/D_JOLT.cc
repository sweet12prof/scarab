...
static constexpr size_t UpperBitPtrBits =
    5;  // This determines that the number of upper bit variations that D-JOLT can handle.
...
static constexpr size_t SignatureBits =
    23;  // Note: if you change this, you may need to re-tune the hash function in siggens.
...
class UpperBitTable {
 ...
 public:
  std::pair<bool, CompressedLineAddress> compress(uint64_t full_address) {
    ...
    const auto exists_pos = std::find_if(table.begin(), table.end(), [upper_bits](const Entry& e) noexcept {
...
class FullyAssociativeLRUTable {
  ...
  size_t find_index_of(const T& key) const {
    ...
    return std::find_if(table.begin(), table.end(),
                        [tag](const Entry& entry) noexcept { return entry.valid && entry.tag == tag; }) -
           table.begin();
  }
...
class FullyAssociativeLRUSet {
 ...
 public:
  ...
  size_t find_lru_index() {
    return static_cast<size_t>(std::max_element(lru_order.begin(), lru_order.end()) - lru_order.begin());
  }
};
...
class WindowBasedStreamPrefetcher {
  ...
  void allocate_training_stream(uint64_t line_address) {
    ...
    training_table.at(lru_index) =
        TrainingStreamEntry{/* valid = */ true, /* start_line_address = */ line_address, /* count = */ 0};
    ...
    for (size_t i = 0; i < TrainingTableSize; ++i) {
      ...
      if (stream.count >= TrainingThreshold) {
        ...
        allocate_monitoring_stream(MonitoringStreamEntry{
            /* valid = */ true,
            /* start_line_address = */ line_address});  // line_address, line_address+1, ... will be prefetched.
        stream.valid = false;
      } else {
...
class SignatureQueue {
 ...
 public:
  ...
  void insert(uint32_t x) noexcept {
    queue.at((head + 1) % N) = x;
    head = (head + 1) % N;
  }
};
...
class D_JOLT_PREFETCHER {
  ...
  SetAssociativeLRUTable<ShortRangePrefetcher_N_Sets, ShortRangePrefetcher_N_Ways, uint32_t,
                         MissTableEntry<ShortRangePrefetcher_N_Vectors, ShortRangePrefetcher_VectorSize>, SigHasher>
      miss_table_1 = {};
  SetAssociativeLRUTable<LongRangePrefetcher_N_Sets, LongRangePrefetcher_N_Ways, uint32_t,
                         MissTableEntry<LongRangePrefetcher_N_Vectors, LongRangePrefetcher_VectorSize>, SigHasher>
      miss_table_2 = {};
  SetAssociativeLRUTable<ExtraMissTable_N_Sets, ExtraMissTable_N_Ways, uint32_t,
                         MissTableEntry<ExtraMissTable_N_Vectors, ExtraMissTable_VectorSize>, SigHasher>
      extra_miss_table = {};
