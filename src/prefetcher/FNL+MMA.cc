...
#define MAXFNL \
  5  // 3 to 6  reaches approximately the same performance, but slightly more accesses to L2 with larger MAXFNL
...
#define PrefCodeBlock(X)                                                                                         \
  new_mem_req(MRT_IPRF, fnlmma_proc_id, X << LOG2(ICACHE_LINE_SIZE), ICACHE_LINE_SIZE, 0, NULL, instr_fill_line, \
              unique_count, 0)
...
void fnlmma_prefetch(uns proc_id, uint64_t v_addr, uint8_t cache_hit, uint8_t prefetch_hit) {
  ...
  if (ShadowMiss) {
    ...
    // Once a block has become worth prefetching, it keeps this status for at least three intervals of PERIODRESET
    // I-Shadow misses
    {
      if (Touched[i])
        if (WorthPF[i] > 0)
    ...
    AheadPredictedBlock = AHEADphist.AheadPredict((v_addr >> 2) ^ (PREVADDR[NSHIFT - 1] << 1));
      ...
      if (NotJustMMA) {
        ...
        if (WorthPF[index] > 0) {
          for (int i = 1; i <= MAXFNL; i++) {
            ...
            if ((WasNotJustFnl(AheadPredictedBlock)) || (i == MAXFNL))
              PrefCodeBlock(pf_Block);
    ...
    if ((Block != (PREVADDR[0] >> 4) + 1) ||
      ...
      if ((PREFCAND[AHEAD.distahead] != 0) & (PREFCAND[AHEAD.distahead] != Block))
        AHEADphist.LinkAhead(Block, PREVADDR[AHEADphist.distahead] ^ (PREVADDR[AHEADphist.distahead + NSHIFT] << 1),
                             cache_hit);
    }
...
void print_fnlmma_stats(uns proc_id) {
  ...
  printf("TOTAL PREFETCHER STORAGE SIZE: %d bytes\n",
         (71 * NBWAYPRED * SIZEWAYNEXTMISS / 8) + ((SIZESHADOWICACHE * (15 + 2)) / 8) + ((FNL_NBENTRIES * 3) / 8) +
